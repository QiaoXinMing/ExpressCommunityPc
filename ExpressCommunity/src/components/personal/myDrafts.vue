<template>
 ...
 <ul class="garagraph clearfix"> 
  <li v-for="(expressArctile,index) in draftsArctileList" :key=index> 
     ...
     <router-link tag="a" :to="{name:'publish',query:{id:expressArctile.arctile_id}}">
...
export default {
  name:'myDrafts',
  created: function() {
    this.getPerDraftsList(this.index.getSession("userId"));
  }, 
  components: {
    pg
  },
  ...
  methods: {
    ...mapActions(["getPerDraftsList","changeDraftsPage","deleteMessage"]),
     del(id,index) {
      let obj = {};
      obj.id = id;
      obj.type = 'drafts';
